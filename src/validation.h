...
private:
    ...
    std::set<uint256> m_tips GUARDED_BY(m_mutex);

public:
    ChainTipSet(const ChainstateManager& chainman) : m_chainman(chainman) {}

    void Update(const uint256& new_blockhash) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    bool Contains(const uint256& blockhash) const EXCLUSIVE_LOCKS_REQUIRED(cs_main);
};
