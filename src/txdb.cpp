...
bool CCoinsViewDB::BatchWrite(CCoinsMap &mapCoins, SpendCoinSet& spend_coins, const uint256 &hashBlock) {
    CDBBatch batch(*m_db);
    size_t count = 0;
    size_t changed = 0;
    ...
    while (!spend_coins.empty()) {
        SpendCoinSet::iterator it = spend_coins.begin();
        CoinEntry entry(&*it);
        batch.Erase(entry);
        spend_coins.erase(it);
        changed++;
        count++;
