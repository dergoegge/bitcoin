...
UtreexoLeafData::UtreexoLeafData(const COutPoint& outpoint, const uint256& blockhash,
                                 const Coin& coin) : UtreexoLeafData(coin)
{
...
UtreexoLeafData::UtreexoLeafData(const Coin& coin)
{
...
CScript UtreexoLeafData::ReconstructP2PKH(const CTxIn& in) const
{
    opcodetype op;
...
CScript UtreexoLeafData::ReconstructP2SH(const CTxIn& in) const
{
    opcodetype op;
...
CScript UtreexoLeafData::ReconstructP2WPKH(const CTxIn& in) const
{
...
CScript UtreexoLeafData::ReconstructP2WSH(const CTxIn& in) const
{
    const auto& prog = in.scriptWitness.stack.back();
    uint256 scriptHash;
...
void UtreexoLeafData::GetOutPoint(COutPoint& outpoint) const
{
    outpoint = COutPoint{m_txid, m_out};
}

void UtreexoLeafData::FillCoinsView(CCoinsViewCache& view) const
{
    Coin coin(CTxOut(m_amount, m_script_pubkey), m_height, m_is_coinbase);
    view.AddCoin(COutPoint{m_txid, m_out}, std::move(coin), false);
}

bool UtreexoLeafData::Reconstruct(const CChain& chain, const CTxIn& in)
{
...
UtxoSetInclusionProof::UtxoSetInclusionProof(const std::vector<UtreexoLeafData>&& elements,
