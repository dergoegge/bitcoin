...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    bool serve_proof_from_index = pfrom.IsUtreexoConn() && gArgs.GetBoolArg("-blockproofindex", false);
    ...
    if (a_recent_block && a_recent_block->GetHash() == pindex->GetBlockHash()) {
        ...
        if (!ReadRawBlockFromDisk(block_data, pindex->GetBlockPos(), m_chainparams.MessageStart(), pfrom.IsUtreexoConn() && !serve_proof_from_index)) {
        ...
        if (!ReadBlockFromDisk(*pblockRead, pindex, m_chainparams.GetConsensus(), pfrom.IsUtreexoConn() && !serve_proof_from_index)) {
            assert(!"cannot load block from disk");
        }
        pblock = pblockRead;
    }
