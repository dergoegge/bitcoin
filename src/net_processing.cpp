...
private:
    ...
    enum ProcessAtmpResult {
        RELAYED,
        POTENTIAL_ORPHAN,
        REJECTED,
    };
    ProcessAtmpResult ProcessMempoolAcceptance(Peer& peer, const CTransactionRef& tx,
...
bool PeerManagerImpl::ProcessOrphanTx(Peer& peer)
{
    ...
    while (CTransactionRef porphanTx = m_orphanage.GetTxToReconsider(peer.m_id)) {
        ...
        switch (ProcessMempoolAcceptance(peer, porphanTx, result)) {
        case POTENTIAL_ORPHAN: continue;
        case RELAYED:
...
PeerManagerImpl::ProcessAtmpResult
PeerManagerImpl::ProcessMempoolAcceptance(Peer& peer, const CTransactionRef& tx,
            const MempoolAcceptResult& atmp_result)
{
    const TxValidationState& state = atmp_result.m_state;

    if (atmp_result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
        ...
        for (const CTransactionRef& removedTx : atmp_result.m_replaced_transactions.value()) {
    ...
    switch (state.GetResult()) {
    case TxValidationResult::TX_MISSING_INPUTS: return POTENTIAL_ORPHAN;
    case TxValidationResult::TX_RESULT_UNSET:
    case TxValidationResult::TX_CONSENSUS:
    case TxValidationResult::TX_RECENT_CONSENSUS_CHANGE:
    case TxValidationResult::TX_INPUTS_NOT_STANDARD:
