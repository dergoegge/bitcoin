    ...
    FuzzedDataProvider fuzzed_data_provider(buffer.data(), buffer.size());
    {
        ...
        if (mtx) {
            ...
            if (in < tx_to.vin.size()) {
                CharaterizeOutput(SignatureHash(script_code, tx_to, in, fuzzed_data_provider.ConsumeIntegral<int>(), ConsumeMoney(fuzzed_data_provider), fuzzed_data_provider.PickValueInArray({SigVersion::BASE, SigVersion::WITNESS_V0}), nullptr));
                ...
                if (mtx_precomputed) {
                    ...
                    CharaterizeOutput(SignatureHash(script_code, tx_to, in, fuzzed_data_provider.ConsumeIntegral<int>(), ConsumeMoney(fuzzed_data_provider), fuzzed_data_provider.PickValueInArray({SigVersion::BASE, SigVersion::WITNESS_V0}), &precomputed_transaction_data));
                }
            }
        }
    }
