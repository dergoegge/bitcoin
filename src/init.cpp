...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-rpcwritereplydelay=<n>", "Test-only: delay (in milliseconds) injected while writing each HTTP reply, used to expose the optimistic-send race (default: 0)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::RPC);
    argsman.AddArg("-rpcsendcap=<n>", "Test-only: cap (in bytes, 0 = unlimited) on how much HTTP response data is sent per socket write, used to expose the optimistic-send race (default: 0)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::RPC);
