...
static RPCHelpMan settxfee()
{
    return RPCHelpMan{"settxfee",
                "\nSet the transaction fee rate in " + CURRENCY_UNIT + "/kvB for this wallet. Overrides the global -paytxfee configuration option.\n"
...
static RPCHelpMan setfeerate()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (!result.empty()) obj.pushKV("result", result);
            if (!error.empty()) obj.pushKV("error", error);
            return obj;
        },
    };
}
