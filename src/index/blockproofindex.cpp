...
// The following options are meant for testing/verification purposes and can
// slow down the index.
//
...
struct DBVal {
    ...
    {
        READWRITE(obj.m_proof_pos, obj.m_undo_pos, obj.m_proof_size,
                  obj.m_undo_size);
    }
};
...
BlockProofIndex::BlockProofIndex(size_t n_cache_size, bool f_memory,
                                 bool f_wipe)
{
    ...
    m_db = std::make_unique<BaseIndex::DB>(path / "db", n_cache_size, f_memory,
                                           f_wipe);
    m_proof_fileseq =
        std::make_unique<FlatFileSeq>(path, "proof", PROOF_FILE_CHUNK_SIZE);
    m_undo_fileseq =
        std::make_unique<FlatFileSeq>(path, "undo", PROOF_FILE_CHUNK_SIZE);
    fs::path forest_path =
        gArgs.GetDataDirNet() / "indexes" / GetName() / "forest.dat";
    ...
    m_pollard =
        std::make_unique<utreexo::Pollard>(roots, m_forest->NumLeaves());
#endif
}
...
bool BlockProofIndex::ComputeProofForLeaves(
    ...
    const std::vector<UtreexoLeafData>&& leaves, UtxoSetInclusionProof& proof)
{
    LOCK(m_forest_mutex);
...
bool BlockProofIndex::LookupUndoBatch(int height,
                                      utreexo::UndoBatch& undo) const
{
    std::pair<uint256, DBVal> value;
    if (!m_db->Read(DBHeightKey{height}, value)) {
        return false;
    ...
    CAutoFile file(m_undo_fileseq->Open(value.second.m_undo_pos), SER_DISK,
                   CLIENT_VERSION);
    if (file.IsNull()) {
        return error("%s: Failed to open undo file %d", __func__,
                     value.second.m_undo_pos.nFile);
    }

    std::vector<uint8_t> bytes;
    bytes.resize(value.second.m_undo_size);
    file.read(AsWritableBytes(Span{bytes}));
...
bool BlockProofIndex::CommitInternal(CDBBatch& batch)
{
    ...
    CAutoFile undo_file(m_undo_fileseq->Open(m_next_undo_pos), SER_DISK,
                        CLIENT_VERSION);
    if (undo_file.IsNull()) {
        return error("%s: Failed to open undo file %d", __func__,
                     m_next_undo_pos.nFile);
    }
    if (!FileCommit(undo_file.Get())) {
        return error("%s: Failed to commit undo file %d", __func__,
                     m_next_undo_pos.nFile);
    }
...
size_t BlockProofIndex::WriteUndoBatchToDisk(FlatFilePos& pos,
                                             const utreexo::UndoBatch& undo)
{
    ...
    if (pos.nPos + undo_bytes.size() > MAX_PROOF_FILE_SIZE) {
        CAutoFile last_file(m_undo_fileseq->Open(pos), SER_DISK,
                            CLIENT_VERSION);
        if (last_file.IsNull()) {
            LogPrintf("%s: Failed to open filter file %d\n", __func__,
                      pos.nFile);
    ...
    fileout.write(AsBytes(Span{undo_bytes}));
...
bool BlockProofIndex::WriteBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    if (!ReconstructLeavesFromTransactions(active_chain, block.vtx, coins,
                                           in_skip, true, target_leaves)) {
    ...
    if (!m_forest->Prove(block_proof, target_hashes)) {
        return error("%s failed to create proof for block %d", __func__,
                     pindex->nHeight);
    }

    UtxoSetInclusionProof proof(std::move(target_leaves),
                                std::move(block_proof));
    size_t bytes_written = WriteProofToDisk(m_next_proof_pos, proof);
    if (bytes_written == 0) {
        return error("%s failed to write proof to disk for block %d", __func__,
                     pindex->nHeight);
    }
    ...
    if (!m_pollard->Verify(block_proof, sorted_target_hashes)) {
        return error("%s failed to verify proof for block %d", __func__,
                     pindex->nHeight);
    }
#endif
    ...
    if (undo_bytes_written == 0) {
        return error("%s failed to write undo to disk for block %d", __func__,
                     pindex->nHeight);
    }

#if VERIFY_UNDO
    if (!m_forest->Undo(undo)) {
        return error("%s failed to undo the accumulator for block %d", __func__,
                     pindex->nHeight);
    }

    utreexo::UndoBatch unused_undo;
    if (!m_forest->Modify(unused_undo, leaves,
                          proof.GetProof().GetSortedTargets())) {
        return error("%s failed to modify the accumulator for block %d",
                     __func__, pindex->nHeight);
    }
#endif
    ...
    DBVal value{m_next_proof_pos, m_next_undo_pos,
    ...
    if (!m_db->Write(DBHeightKey{pindex->nHeight}, val)) {
        return error(
            "%s failed to write to the proof position to the db at height %d",
            __func__, pindex->nHeight);
    }
                             ...
                             const CBlockIndex* new_tip)
{
    ...
    for (int height = new_tip->nHeight; height <= current_tip->nHeight;
         ++height) {
